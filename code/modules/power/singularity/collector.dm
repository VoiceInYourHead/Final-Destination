...
/obj/machinery/power/rad_collector/proc/collector_break()
	if(P && P.air_contents)
		...
		if(T)
			...
			fragmentate(T, 2, 4, list(/obj/item/projectile/bullet/pellet/fragment/tank/small = 3, /obj/item/projectile/bullet/pellet/fragment/tank = 1), name)
			cell_explosion(T, 100, 100)
			QDEL_NULL(P)
	disconnect_from_network()
	stat |= BROKEN
	melted = TRUE
