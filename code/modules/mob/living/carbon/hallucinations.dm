...
/datum/hallucination/talking/start()
	...
	for(var/mob/living/talker in oview(holder))
		...
		if(prob(80))
			...
			var/list/phrases = list("Эй.","Эй?","Съеби.","Съебись.","Ты че делаешь?","Где твоя карта?","Ты че наделал.")
			if(holder.hallucination_power > 50)
				phrases += list("Зачем заходил?","Не трогай меня.","Ты отсюда уже не выйдешь.", "Уёбище.", "Хуесос.", "Неудачник.","Сдохни.","АЛЕ.","Ну что за уебок.","Тебя на [pick("мостик","кухню","свидание","казнь",)] зовут.","ПОЖАР!","ЗОМБИ!","ПРЕДАТЕЛЬ!"," УРА-А-А-А!!!")
