...
/decl/cultural_info/faction/scg/army
	name = FACTION_ARMY
	description = "The Sol Central Government Army is the SCG's main land force. The Army is responsible for \
	handling any ground conflicts and many peacekeeping missions that the SCG may be engaged in. Smaller than the SCG Fleet, \
	with only a partial full-time component, much of the Army is composed of reservists and, in times of large scale war, \
	...
	While not as famous or as glamorous as the Fleet, the Army is never the less a respected part of the military, \
	though one which is often looked down on by the Fleet because of their reliance on reservists."
