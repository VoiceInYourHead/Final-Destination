...
/obj/machinery/computer/ship/sensors_passive/proc/reveal_contacts(var/mob/user)
	if(user && user.client)
		for(var/key in contact_datums)
			var/datum/overmap_contact/record = contact_datums[key]
			if(record)
...
/obj/machinery/computer/ship/sensors_passive/proc/hide_contacts(var/mob/user)
	if(user && user.client)
		for(var/key in contact_datums)
			var/datum/overmap_contact/record = contact_datums[key]
			if(record)
...
/obj/machinery/computer/ship/sensors_passive/Process()
	..()
	update_sound()
	if(!linked)
		return
...
/obj/machinery/computer/ship/sensors_passive/attackby(var/obj/item/I, var/mob/user)
	. = ..()
...
/obj/machinery/computer/ship/sensors_passive/proc/remove_tracker(var/obj/item/ship_tracker/tracker)
	trackers -= tracker
