...
/obj/machinery/computer/ship/particle_lance/proc/fire_at_sector(var/z_level, var/target_fore_dir, var/target_dir, var/obj/effect/overmap/target, var/firing_on_planet = FALSE)
...
/obj/machinery/computer/ship/particle_lance/proc/handle_beam_damage(var/turf/s, var/d, var/killing_floor = FALSE)
	...
	for(var/turf/T in getline(s,get_target_turf(s, d)))
		...
		if(T.density && !killing_floor)
			sleep(beam_speed)
			explosion(T, 2, EX_ACT_DEVASTATING,adminlog = 0, turf_breaker = TRUE)
			if(T)
				T.ex_act(1,TRUE)
