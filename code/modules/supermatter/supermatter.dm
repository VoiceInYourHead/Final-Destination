...
/obj/machinery/power/supermatter/Process()
	...
	if(damage > explosion_point)
		if(!exploded)
			if(!istype(L, /turf/unsimulated/floor/exoplanet/clouds) && (L.z in GLOB.using_map.station_levels))
				announce_warning()
			explode()
		...
		if(!istype(L, /turf/unsimulated/floor/exoplanet/clouds) && ((world.timeofday - lastwarning) >= WARNING_DELAY * 10) && (L.z in GLOB.using_map.station_levels))
			announce_warning()
	else
	...
	if(!istype(L, /turf/unsimulated/floor/exoplanet/clouds))
		env = L.return_air()
