...
/obj/machinery/shieldgen/proc/create_shields()
	for(var/turf/target_tile in range(8, src))
		if ((istype(target_tile,/turf/unsimulated/floor/exoplanet/clouds)|| istype(target_tile, /turf/simulated/open)) && !(locate(/obj/machinery/shield) in target_tile))
	...
	for(var/turf/above in range(8, GetAbove(src)))//Probably a better way to do this.
		if ((istype(above,/turf/unsimulated/floor/exoplanet/clouds)|| istype(above, /turf/simulated/open)) && !(locate(/obj/machinery/shield) in above))
			if (malfunction && prob(33) || !malfunction)
...
/obj/machinery/shieldgen/attackby(obj/item/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		...
		if(anchored)
			...
			if(istype(get_turf(src), /turf/unsimulated/floor/exoplanet/clouds)) return //No wrenching these in space!
			playsound(src.loc, 'sound/items/Ratchet.ogg', 100, 1)
