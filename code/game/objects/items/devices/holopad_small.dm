...
/obj/item/device/holopad/proc/incall(obj/item/device/holopad/caller)
	...
	desc = "[initial(desc)] Входящий вызов от [caller.getName()]."
...
/obj/item/device/holopad/proc/placeCall(mob/user)
	...
	if(target.incall(src))
		...
		to_chat(user, SPAN_NOTICE("Вызываем [sanitize(abonent.getName(1))]"))
	else
		to_chat(user, SPAN_WARNING("На данный момент, устройство недоступно"))
...
/obj/item/device/holopad/proc/acceptCall()
	if(call_state == CALL_RINGING)
		if(abonent && abonent.call_state == CALL_CALLING)
			...
			audible_message("<span class='name'>[voice]</span> передаёт, \"Соединение установлено\"", hearing_distance = 1)
		else
...
/obj/item/device/holopad/proc/hangUp(remote = 0)
	...
	audible_message(SPAN_WARNING("Соединение разорвано!"), hearing_distance = 4)
	call_state = CALL_NONE
	icon_state = initial(icon_state)
	desc = initial(desc)
	abonent = null
...
/obj/item/device/holopad/proc/receive(speaking, mob/user)
	...
	for(var/mob/M in listening)
		to_chat(M, "<span class='name'>[voice]</span> передаёт, \"[speaking]\" ")
