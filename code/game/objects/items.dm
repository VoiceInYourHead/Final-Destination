...
//	causeerrorheresoifixthis
	...
	var/list/blacklist = null //blacklist for suitstorage
	...
	switch(slot)
		...
		if(slot_s_store)
			...
			if(!H.wear_suit.allowed)
				...
				return 0
			if( !(istype(src, /obj/item/modular_computer/pda) || istype(src, /obj/item/pen) || is_type_in_list(src, H.wear_suit.allowed)) || is_type_in_list(src, H.wear_suit.blacklist) )
				return 0
		if(slot_handcuffed)
...
/obj/item/proc/on_disarm_attempt(mob/target, mob/living/attacker)
	...
	if(attacker.a_intent == I_HELP)
		return 0
	attacker.apply_damage(force/2, damtype, attacker.hand ? BP_L_HAND : BP_R_HAND, used_weapon = src)
