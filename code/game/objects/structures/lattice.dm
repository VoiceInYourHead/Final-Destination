...
/obj/structure/lattice/Initialize(mapload, var/new_material)
	...
	if(!(istype(src.loc, /turf/unsimulated/floor/exoplanet/clouds) || istype(src.loc, /turf/simulated/open) || istype(src.loc, /turf/unsimulated/floor/plating/rainworld/fake_openspace)))
...
/obj/structure/lattice/on_update_icon()
	...
	for (var/direction in GLOB.cardinal)
		...
		if(locate(/obj/structure/lattice, T) || locate(/obj/structure/catwalk, T))
			...
			if(!(istype(get_step(src, direction), /turf/unsimulated/floor/exoplanet/clouds)) && !(istype(get_step(src, direction), /turf/simulated/open)))
				dir_sum += direction
