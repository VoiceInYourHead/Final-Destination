...
/datum/controller/subsystem/zcopy/proc/calculate_zstack_limits()
	...
	var/start_zlev = 1
	for (var/z in 1 to world.maxz+5) //oa oaa oaaaaa oaaa
		if (!HasAbove(z))
...
/datum/controller/subsystem/zcopy/fire(resumed = FALSE, no_mc_tick = FALSE)
	...
	while (qt_idex <= length(curr_turfs))
		...
		if (T.below.z_eventually_space)
			...
			if ((T.below.z_flags & ZM_MIMIC_OVERWRITE) || T.below.type == /turf/unsimulated/floor/exoplanet/clouds)
		...
		for (var/thing in T.below)
			...
			if(length(zlev_maximums) < T.z)
				log_and_message_admins("ТЫ ЕБАЛАЙ У ТЕБЯ КОСТЫЛЬ ZCOPY СЛОМАЛСЯ: [length(zlev_maximums)] < [T.z] ([T.x][T.y])", location=T)
...
/client/proc/analyze_openturf(turf/T)
	...
	var/list/out = list(
		...
		"<b>Above space:</b> Apparent [T.z_eventually_space ? "Yes" : "No"], Actual [is_above_space ? "Yes" : "No"] - [T.z_eventually_space == is_above_space ? "OK" : "MISMATCH"]",
...
/datum/controller/subsystem/zcopy/proc/debug_fmt_thing(atom/A, list/out, turf/original)
	if (istype(A, /atom/movable/openspace/mimic))
		...
		if (A == original)
			return "<li>\icon[A] <b>\[Turf\]</b> plane [A.plane], layer [A.layer], depth [FMT_DEPTH(A:z_depth)], Z-level [A.z] - [A] ([A.type]) - ["SELF"]</li>"
		else	// foreign turfs - not visible here, but sometimes good for figuring out layering -- showing these is currently not enabled
			return "<li>\icon[A] <b>\[Turf\]</b> <em>["plane [A.plane], layer [A.layer], depth [FMT_DEPTH(A:z_depth)], Z-level [A.z] - [A] ([A.type])"]</em> - ["FOREIGN"]</em></li>"
		...
		return "<li>\icon[A] <b>\[Shadower True Overlay\]</b> plane [A.plane], layer [A.layer] - ["VIRTUAL"]</li>"
