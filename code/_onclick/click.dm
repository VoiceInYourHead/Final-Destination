...
/mob/living/proc/check_psi_grab(var/obj/item/grab/grab)
	if(psi && ismob(grab.affecting))
		if(a_intent == I_HURT)
			INVOKE_PSI_POWERS(src, psi.get_grab_powers(SSpsi.faculties_by_name_new["Hyloforia"]), grab.affecting)
		if(a_intent == I_GRAB)
			INVOKE_PSI_POWERS(src, psi.get_grab_powers(SSpsi.faculties_by_name_new["Teleplexy"]), grab.affecting)
