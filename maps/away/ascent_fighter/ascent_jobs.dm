#define WEBHOOK_SUBMAP_LOADED_ASCENT_FIGHTER "webhook_submap_ascent_fighter"

// Submap datum and archetype.
/decl/webhook/submap_loaded/ascent_fighter
	id = WEBHOOK_SUBMAP_LOADED_ASCENT_FIGHTER

/decl/submap_archetype/ascent_fighter
	descriptor = "Ascent fighter"
	map = "Ascent Fighter"
	blacklisted_species = null
	whitelisted_species = null
	crew_jobs = list(
		/datum/job/submap/ascent_fighter,
		/datum/job/submap/ascent_fighter/alate,
//		/datum/job/submap/ascent/drone
	)
	call_webhook = WEBHOOK_SUBMAP_LOADED_ASCENT_FIGHTER

/datum/submap/ascent_fighter
	var/gyne_name

/datum/submap/ascent_fighter/sync_cell(obj/effect/overmap/visitable/cell)
	return

/datum/submap/ascent_fighter/check_general_join_blockers(var/mob/new_player/joining, var/datum/job/submap/job)
	. = ..()
	if(. && istype(job, /datum/job/submap/ascent_fighter))
		var/datum/job/submap/ascent_fighter/ascent_job = job
...
/datum/job/submap/ascent_fighter/is_position_available()
	. = ..()
	if(. && requires_supervisor)
		for(var/mob/M in GLOB.player_list)
			if(!M.client || !M.mind || !M.mind.assigned_job || M.mind.assigned_job.title != requires_supervisor)
				continue
			var/datum/job/submap/ascent_fighter/ascent_job = M.mind.assigned_job
			if(istype(ascent_job) && ascent_job.owner == owner)
				return TRUE
		return FALSE

/datum/job/submap/ascent_fighter/handle_variant_join(var/mob/living/carbon/human/H, var/alt_title)
	...
	var/datum/submap/ascent_fighter/cutter = owner
	if(!istype(cutter))
...
/datum/job/submap/ascent_fighter/alate
	...
	outfit_type = /decl/hierarchy/outfit/job/ascent/pilot
	min_skill = list(
		SKILL_EVA = SKILL_TRAINED,
		SKILL_PILOT = SKILL_TRAINED,
		SKILL_HAULING = SKILL_TRAINED,
...
/obj/effect/submap_landmark/spawnpoint/ascent_fighter/alate
	name = "Ascent Pilot"
	movable_flags = MOVABLE_FLAG_EFFECTMOVE

#undef WEBHOOK_SUBMAP_LOADED_ASCENT_FIGHTER
