#define WEBHOOK_SUBMAP_LOADED_ASCENT_INTERCEPTOR "webhook_submap_ascent_interceptor"

// Submap datum and archetype.
/decl/webhook/submap_loaded/ascent_interceptor2
	id = WEBHOOK_SUBMAP_LOADED_ASCENT_INTERCEPTOR

/decl/submap_archetype/ascent_interceptor2
	descriptor = "Ascent interceptor (2)"
	map = "Ascent Interceptor (2)"
	blacklisted_species = null
	whitelisted_species = null
	crew_jobs = list(
		/datum/job/submap/ascent_interceptor2,
		/datum/job/submap/ascent_interceptor2/alate,
//		/datum/job/submap/ascent/drone
	)
	call_webhook = WEBHOOK_SUBMAP_LOADED_ASCENT_INTERCEPTOR

/datum/submap/ascent_interceptor2
	var/gyne_name

/datum/submap/ascent_interceptor2/sync_cell(obj/effect/overmap/visitable/cell)
	return

/datum/submap/ascent_interceptor2/check_general_join_blockers(var/mob/new_player/joining, var/datum/job/submap/job)
	. = ..()
	if(. && istype(job, /datum/job/submap/ascent_interceptor2))
		var/datum/job/submap/ascent_interceptor2/ascent_job = job
...
/datum/job/submap/ascent_interceptor2/is_position_available()
	. = ..()
	if(. && requires_supervisor)
		for(var/mob/M in GLOB.player_list)
			if(!M.client || !M.mind || !M.mind.assigned_job || M.mind.assigned_job.title != requires_supervisor)
				continue
			var/datum/job/submap/ascent_interceptor2/ascent_job = M.mind.assigned_job
			if(istype(ascent_job) && ascent_job.owner == owner)
				return TRUE
		return FALSE

/datum/job/submap/ascent_interceptor2/handle_variant_join(var/mob/living/carbon/human/H, var/alt_title)
	...
	var/datum/submap/ascent_interceptor2/cutter = owner
	if(!istype(cutter))
...
/obj/effect/submap_landmark/spawnpoint/ascent_interceptor2/alate
	name = "Ascent Alate (2)"
	movable_flags = MOVABLE_FLAG_EFFECTMOVE

#undef WEBHOOK_SUBMAP_LOADED_ASCENT_INTERCEPTOR
