...
/decl/webhook/submap_loaded/ascent_interceptor
	id = WEBHOOK_SUBMAP_LOADED_ASCENT_INTERCEPTOR

/decl/submap_archetype/ascent_interceptor
	descriptor = "Ascent interceptor"
	map = "Ascent Interceptor"
	blacklisted_species = null
	whitelisted_species = null
	crew_jobs = list(
		/datum/job/submap/ascent_interceptor,
		/datum/job/submap/ascent_interceptor/alate,
//		/datum/job/submap/ascent/drone
	)
	call_webhook = WEBHOOK_SUBMAP_LOADED_ASCENT_INTERCEPTOR

/datum/submap/ascent_interceptor
	var/gyne_name

/datum/submap/ascent_interceptor/sync_cell(obj/effect/overmap/visitable/cell)
	return

/datum/submap/ascent_interceptor/check_general_join_blockers(var/mob/new_player/joining, var/datum/job/submap/job)
	. = ..()
	if(. && istype(job, /datum/job/submap/ascent_interceptor))
		var/datum/job/submap/ascent_interceptor/ascent_job = job
...
/datum/job/submap/ascent_interceptor/is_position_available()
	. = ..()
	if(. && requires_supervisor)
		for(var/mob/M in GLOB.player_list)
			if(!M.client || !M.mind || !M.mind.assigned_job || M.mind.assigned_job.title != requires_supervisor)
				continue
			var/datum/job/submap/ascent_interceptor/ascent_job = M.mind.assigned_job
			if(istype(ascent_job) && ascent_job.owner == owner)
				return TRUE
		return FALSE

/datum/job/submap/ascent_interceptor/handle_variant_join(var/mob/living/carbon/human/H, var/alt_title)
	...
	var/datum/submap/ascent_interceptor/cutter = owner
	if(!istype(cutter))
...
/obj/effect/submap_landmark/spawnpoint/ascent_interceptor/alate
	name = "Ascent Alate"
